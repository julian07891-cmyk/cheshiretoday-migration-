...
ROUTES = (
    ("/api/send-digest", server.send_digest_now),
    ("/api/trigger-daily-generation", server.trigger_daily_generation),
)
...
def test_cost_operation_has_one_authenticated_route(path, endpoint):
...
def test_unauthenticated_cost_operation_starts_no_business_work(
    monkeypatch,
    path,
    _endpoint,
):
    ...
    email_service = UntouchedCollaborator()
    rss_feed = UntouchedCollaborator()
    perplexity = UntouchedCollaborator()
    generation_called = False

    async def fail_if_generation_starts(*_args, **_kwargs):
        nonlocal generation_called
        generation_called = True
        raise AssertionError("article generation must not start before authentication")

    monkeypatch.setattr(server, "db", database)
    monkeypatch.setattr(server, "email_service", email_service)
    monkeypatch.setattr(server, "daily_article_generation", fail_if_generation_starts)
