...
- Latest commit recorded:
  - `12260c5` — “Stabilise homepage system: 40/40/20 ratio, Top Stories 8 layout alignment, editorial policy filtering, API cache fix, affiliate sidebar correction”
...
--------------------------------------------------
MEMORY SNAPSHOT (PERSIST THIS IN NEW CHAT)
--------------------------------------------------

Assistant name preference:
- Call the assistant: Boren

Project identity:
- Cheshire Today is being rebuilt as a hybrid local + business/finance + AI/tech authority publication.
- Positioning: “local economic intelligence platform for Cheshire”.
- Avoid crime-heavy / sensational strategy.

Non-negotiable workflow rules:
- One terminal command at a time.
- No manual file edits (use terminal scripts/commands only).
- Always check current state before modifying.
- Prefer `grep` (no `rg` installed).
- Prefer running commands from project root (avoid `cd frontend`/`cd backend` when possible).
- Local verification method MUST be:
  - `npm run build`
  - `npx serve -s build`
  - Do NOT rely on `npm start` (dev server issue: “compiled successfully” but no TCP listener).

Render / deployment protocol:
- Render auto-deploy is disabled (manual deploy only).
- Work is done on staging/migration; production domain swap happens only when ready.

Monetisation constraints:
- Affiliate-first monetisation.
- Guides/authority pages can exist in DB/backend, but must NOT be promoted/visible until affiliate approvals.
- Non-Amazon monetisation is feature-flagged OFF:
  - `frontend/src/config/features.js` => `NON_AMAZON_MONETISATION_ENABLED: false`
- Keep nav minimal: Local / Business / UK. AI/Tech surfaced via homepage sections/guides later (not top nav).

Homepage logic / layout requirements:
- Keep current layout: smaller hero + sidebar.
- Top Stories target: 8 items (for left/right column alignment).
- Enforce content ratios (40/40/20: Local / Business+Finance / AI-Tech) via selection logic.
- Dedupe across homepage slots is required.
- Backend `/api/articles` supports `with_total=1` and returns envelope `{articles,total,...}`.

Known current development issue:
- CRACO dev server starts and prints URLs but no listener opens on 3000/3010 (connection refused).
- Static build + serve works; dev server is deprioritised unless needed later.

Git / code state checkpoint:
- Recent commit (confirmed): 12260c5
  Message: "Stabilise homepage system: 40/40/20 ratio, Top Stories 8 layout alignment, editorial policy filtering, API cache fix, affiliate sidebar correction"

New chat instruction (use this verbatim):
...
Resume from 'Next steps to production'."
