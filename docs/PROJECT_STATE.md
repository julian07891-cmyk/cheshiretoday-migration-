...
Combined affected regression passed **403 tests, 18 pre-existing deprecation warnings, zero failures**. `backend/server.py` compilation and `git diff --check` passed, and the complete production/test diff was reviewed against the D-1 boundary. D-1 was committed as `34ecf74 Add provider suppression delivery eligibility`, pushed to `full-scrape-prod`, deployed Live on the exact commit and verified through `https://cheshiretoday.co.uk/api/health` with HTTP 200.

The authoritative 24 September Resend suppression export contained **815 unique suppressions: 785 bounce and 30 complaint**. Privacy-safe production reconciliation matched all **815/815** to subscriber records with zero unmatched and zero duplicate database matches; **583 were locally active and 232 inactive/legacy**. A private pre-reconciliation backup of all 815 matched records was validated with `0600` permissions and an exact 815-ID match to the intended production targets. The first approved transaction deliberately aborted when exact post-write verification exposed BSON millisecond precision loss for 60 Resend timestamps carrying sub-millisecond precision; an immediate read-only check proved production remained **0 reconciled / 815 pending / 0 non-exact**. Timestamp handling was corrected to BSON millisecond precision and independently round-trip tested inside an aborted disposable transaction.

The corrected approved transaction then reported **815 matched, 815 modified, 815 exact post-write and transaction committed**. Independent post-commit verification returned **815 exactly reconciled, 0 requiring change and 0 non-exact**. Global production aggregates independently confirmed **815 provider-suppressed records: 583 active, 232 inactive/legacy, 785 Resend bounce, 30 Resend complaint, zero unexpected source and zero unexpected reason**. No subscriber was unsubscribed or reactivated, no newsletter preference was changed, no Resend suppression was removed, and no provider/DNS configuration was changed. The 583 locally active records remain subscribed in Cheshire Today lifecycle state but are excluded from subscriber-content delivery by the live D-1 eligibility guard. See [CT-DEC-022](DECISION_REGISTER.md#ct-dec-022--separate-provider-suppression-from-subscription-lifecycle), [Newsletter Architecture](ARCHITECTURE/NEWSLETTER.md#provider-suppression-and-delivery-eligibility--ct-dec-022) and [Newsletter Operations](OPERATIONS/NEWSLETTER_OPERATIONS.md#provider-suppression-reconciliation--ct-dec-022).
