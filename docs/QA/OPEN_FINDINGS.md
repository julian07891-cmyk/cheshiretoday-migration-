...
- **Original severity:** High
- **Current severity:** High
- **Area:** Imported article body quality / RSS completeness
- **Original finding:** A Guardian continuation-ended RSS preview became public as article content with a terminal `Continue reading...`; `_clean_html()` also removed block tags without separators, producing joined boundaries analogous to `bursaryLike` and `ever.“It`. A bounded production audit found five current Guardian-derived records with case-insensitive `Continue reading`; this was not a lifetime count.
- **Current status:** **IMPLEMENTED, DEPLOYED — NATURAL ACCEPTANCE PARTIAL.** The core incomplete-preview routing and bounded HTML-boundary behaviour are naturally verified. The complete-replacement path is not naturally exercised. No production defect attributable to `bbc526c` is confirmed.
- **Root cause:** Guardian RSS can contain continuation-ended previews; tag deletion lost structural boundaries; continuation sanitisation missed ellipsis variants and could remove evidence before completeness classification; incomplete raw fallback could remain public-eligible through `manual_review_without_ai`. This was not an AI-authored-content defect.
- **Current-code evidence:** Commit `bbc526c` preserves BeautifulSoup block boundaries, normalises LF/CRLF/CR, recognises/removes supported terminal-only continuation markers, classifies incomplete raw previews before sanitisation, conservatively distinguishes replacements, retains the 1,000-character completeness floor, and forces known incomplete fallback content into existing hidden Manual Review. No full Guardian scraping/fetching was introduced.
- **Test evidence:** 44 focused tests; 130 importer/Manual Review/scheduler tests passed with 12 skipped; 104 memory-observability/Editorial-Similarity-isolation/cleanup tests passed; `compileall` and `git diff --check` passed. Complete unstaged and staged reviews were approved with no Blocker, Material or Minor finding at the commit gate. This is implementation evidence, not production evidence.
- **Deployment evidence:** Render deployment `dep-damij1btqb8s73fvesp0` ran exact SHA `bbc526c` on `cheshiretoday-migration-` (`srv-d5virmm3jp1c73c9d6tg`), instance `klzb8`, Standard 1 CPU/2 GB. It started 13:07:33 BST on 18 September 2026, built at 13:09:25, completed startup at 13:10:10 and became live at 13:10:17 (2m44s). Health was HTTP 200; no attributable traceback, fatal exception, OOM, exit 137, restart loop, Bad Gateway or material 5xx was observed.
- **Natural production evidence:** The natural 18:00 BST run completed once at 18:01:58 in 118.50 seconds. Guardian Money/Housing/Science/UK feeds produced 26/18/26/44 candidates. Output was Cheshire 3, UK 3, Business 2, Tech 4, Sports 0, 15 hybrid-generation results, zero cleanup removals and about 302.0 MB final RSS. Record `6aad6e3245d0658a103d921e`, “Polanski request to postpone Greens’ conference to focus on byelection refused by party”, was retained hidden in Manual Review with source metadata/URL and reason `Source-derived content is an incomplete RSS preview and needs manual review before publication.`; it was absent from public search and not archived/deleted. Bounded inspection of it and public Guardian record `6aad6e7245d0658a103d9228`, “Open AI and the million-dollar maths problem - video”, found separated structure and no supported terminal marker or analogous joined boundary.
- **Remaining gaps:** **COMPLETE-REPLACEMENT PATH NOT NATURALLY EXERCISED.** Five pre-deployment Guardian records remain untouched and require separate editorial repair. The known ID is `6aa9799c211c945c4231deda`, “‘Just be aware of what you’re spending’: budget hints for students”. The other known headlines are “Burnham hosts Ireland’s taoiseach as he bids for UK exemptions from EU’s ‘Made in Europe’ plan – UK politics live”, “Pensioners are ‘big winners’ with triple-lock set to rise by 3.9%, lifting state pension to £13,000 – business live”, “MPs to vote again on assisted dying as bill returns to parliament – UK politics live”, and “Hospitality industry fears jobs at risk from plan to give mayors powers to impose ‘tourist tax’”; their IDs are not asserted. `/api/import-real-news` is a separate follow-up risk and was neither broadened nor production-tested here.
- **Closure criteria:** Observe a natural continuation-ended candidate receiving a genuinely distinct complete replacement that passes existing quality controls and becomes public; separately decide/complete repair of the five historical records. Do not manufacture a candidate or block unrelated work while awaiting it.
- **Owner/documentation responsibility:** Editorial/import owner and backend QA owner; [Article Pipeline](../ARCHITECTURE/ARTICLE_PIPELINE.md).
- **Sources:** Git `bbc526c`; deployment `dep-damij1btqb8s73fvesp0`; authenticated natural run and bounded Admin/public inspection on 18 September 2026.
...
## CT-QA-2026-007 — Legitimate generic newsletter-management entry displays invalid-link warning

- **Severity:** Medium (at discovery).
- **Original status:** OPEN — ROOT CAUSE CONFIRMED; NOT IMPLEMENTED.
- **Current status:** **IMPLEMENTED, DEPLOYED AND PRODUCTION-VERIFIED**.
- **Root cause:** The shared frontend entry model conflated a legitimate no-token generic entry with malformed/invalid supplied credentials on `/newsletter/preferences`, `/unsubscribe` and `/newsletter/reactivate`.
- **Implementation:** `e6408133c48a98b2e22e8cf22a958bce7a922716` — `Fix generic newsletter management entry states`. Exactly four frontend files changed: `frontend/src/components/NewsletterSignupSimplification.test.jsx`, `frontend/src/components/SecureNewsletterManagementPages.jsx`, `frontend/src/components/SecureNewsletterManagementPages.test.jsx` and `frontend/src/services/secureNewsletterManagement.js` (89 insertions, 12 deletions).
- **Scope:** Explicit generic/token/invalid/retired classification gives clean generic entries neutral request-link UI. Malformed, empty or overlong supplied credentials remain invalid; query tokens remain unsupported, retired links remain retired, valid secure-token flows and fragment capture/scrubbing remain unchanged. No automatic management request was introduced. No backend/API, DB/migration, token, challenge, rate-limit, anti-enumeration or mutation contract changed.
- **Verification:** Six relevant frontend suites / 96 tests passed and the production frontend build passed; see [Test History](TEST_HISTORY.md). The existing outdated Browserslist warning was unrelated and nonblocking.
- **Deployment:** Auto-Deploy `dep-dao287ajnfac739ab80g` on `cheshiretoday-migration-` (`srv-d5virmm3jp1c73c9d6tg`) ran the exact implementation SHA, instance `cqsqq`, Live / Deploy succeeded. On 20 September 2026 (BST), start was 19:21:17, build success 19:22:55, application startup complete 19:23:45 and Live 19:23:47. Build, backend startup, Mongo/index and scheduler registrations were normal. Bounded logs showed no attributable traceback, OOM, exit 137, crash or restart loop. Health returned HTTP 200 with `{"status":"healthy","service":"cheshire-news"}`; homepage returned HTTP 200.
- **Production UX:** All three generic routes returned HTTP 200 with an email field and request button, without “This link is not valid”. Preferences displayed: “Enter the email address associated with your subscription. If eligible, we'll send you a secure link to manage your newsletter preferences.” Unsubscribe displayed: “Enter the email address associated with your subscription. If eligible, we'll send you a secure link to confirm unsubscribe. Requesting a link does not unsubscribe you.” Reactivation displayed: “Enter the email address associated with your subscription. If eligible, we'll send you a secure link to reactivate your newsletters.”
- **Mobile:** All three routes passed at 390×844: readable copy, usable fields, document width 390, no clipping, overlap, horizontal overflow or invalid-link warning.
- **Evidence boundary:** No form was submitted, management email requested, challenge intentionally generated or subscriber mutated. Live browser network capture was unavailable: zero-auto-request behaviour is supported by the exact deployed code, regression assertions and no form submissions, not independent live network proof. This does not block the bounded presentation acceptance.
