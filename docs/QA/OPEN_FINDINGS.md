...
- **Current status:** **OPEN — EVENT-IDENTITY FEATURE DESIGN REQUIRED.** The normal scheduled hybrid importer is not evidenced to be reimporting archived records. Two labelled-pair calibration rounds show that the current scorer and tested composite are not safe for routing; event-level identity across publishers, localisations and formats requires bounded deterministic feature design before any publication-state change.
...
- **Remaining gap:** Conduct a bounded read-only design review for a deterministic event-anchor layer against the full 27-pair regression matrix. The review should assess high-specificity named entities, normalized event-defining quantities, event-stage compatibility, article/video/live-blog/analysis format guards and exact locality phrase/token matching. No extraction rule is approved. Preserve the bounded corpus and shortlist and introduce no embeddings, per-comparison LLM call, full archive scan, unbounded content comparison or broad Mongo materialisation; `QA-OPS-001` remains separate.
...
- **Roadmap mapping:** [Roadmap Master](../ROADMAP_MASTER.md), bounded deterministic event-anchor design review.
- **Sources:** Read-only production active/archive ledger and two labelled-pair calibration rounds reconciled 14 August 2026; [Project State](../PROJECT_STATE.md); [Editorial Similarity architecture](../ARCHITECTURE/EDITORIAL_SIMILARITY.md).
