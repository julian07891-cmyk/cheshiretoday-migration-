...
> Privacy-safe repository copy. Derived from the byte-exact local archive.
> Personal email addresses, private filesystem paths and any secret values have
> been replaced with explicit redaction markers. The original local archive is
> preserved separately and is identified by SHA-256 in `SOURCE_REGISTER.md`.
...
- a message was sent to `[REDACTED_EMAIL]` as a fallback because the support route was not working
...
- `[REDACTED_LOCAL_PATH] 2.csv`
- Valid unique emails found: `5022`
- Already in subscribers: `0`
- New imported: `5022`

2. Second import source:
- `[REDACTED_LOCAL_PATH] Documents/com~apple~CloudDocs/Downloads/16kemailslist2023dec.csv`
...
- `POST /api/send-digest-test?test_email=[REDACTED_EMAIL]&use_preview_links=false`

Verified result:
- `success: true`
- one Daily Brief was sent to `[REDACTED_EMAIL]`
...
2. Daily Brief live send infrastructure works and was successfully tested to `[REDACTED_EMAIL]`.
...
- `RESEND_ENABLED=true`
- `RESEND_API_KEY=...`
- `RESEND_FROM_EMAIL=[REDACTED_EMAIL]`
...
- sends an internal email notification to `[REDACTED_EMAIL]`,
- records whether the notification was successfully sent.

Live production test confirmed:
- lead capture worked,
- MongoDB save worked,
- notification email sent successfully to `[REDACTED_EMAIL]`,
...
5. An internal notification email is sent to `[REDACTED_EMAIL]`.
...
5. internal email goes to `[REDACTED_EMAIL]`,
...
```text
https://www.dhl.com/gb-en[REDACTED_LOCAL_PATH]
```
...
- 1.3% for existing customers.
- 30-day cookie window.
- Contact: Chris at `[REDACTED_EMAIL]`.
...
Awin listing supplied by user included many finance/insurance[REDACTED_LOCAL_PATH] options.
...
```bash
curl -sS "https://cheshiretoday.co.uk/api/articles?limit=120" -o [REDACTED_LOCAL_PATH]
```
...
The guide upsert endpoint required a full page payload including title, not `sections_append`. The guide was fetched to `[REDACTED_LOCAL_PATH]`, amended in Python, then posted back to:
...
/usr/bin/git config user.email "[REDACTED_EMAIL]"
```
