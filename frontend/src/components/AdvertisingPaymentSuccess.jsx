...
import { CheckCircle, Loader2, XCircle, ArrowLeft, Clock, Megaphone } from "lucide-react";
import { getApiUrl } from "../utils/api";
import { Button } from "./ui/button";
import { Card, CardContent } from "./ui/card";

const AdvertisingPaymentSuccess = () => {
  const [searchParams] = useSearchParams();
  const sessionId = searchParams.get("session_id");
  const [status, setStatus] = useState("checking");
  const [message, setMessage] = useState("Verifying your advertising payment...");
  ...
  const pollPaymentStatus = async () => {
    ...
    if (attempts >= maxAttempts) {
      setStatus("failed");
      setMessage("Payment verification timed out. Please check your email for confirmation or contact news@cheshiretoday.co.uk.");
      return;
    }

    try {
      const response = await fetch(`${getApiUrl()}/api/advertising/payment-status/${sessionId}`);
      const data = await response.json();

      if (data.payment_status === "paid" || data.status === "completed") {
        setStatus("success");
        setMessage(data.message || "Payment successful. Your advert is pending Cheshire Today review.");
        return;
      }
      ...
      console.error("Error checking advertising payment status:", error);
      setAttempts(prev => prev + 1);
      setTimeout(pollPaymentStatus, pollInterval);
    }
  };
            ...
            <>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/30 rounded-lg p-4 mb-6">
                ...
                <p className="text-sm text-amber-600 dark:text-amber-500 mt-2">
                  Cheshire Today will review your advert before it goes live. We may contact you if we need a logo, image, wording change or placement clarification.
                </p>
              </div>
...
export default AdvertisingPaymentSuccess;
