...
function HomePage() {
  ...
  const buildPriorityCheshireArticles = () => {
    ...
    const allCheshire = articles.filter(
      (a) =>
  ...
  const secondaryCheshireArticles = articles.filter(
    (a) => a.is_secondary_cheshire && !a.is_priority_cheshire,
  );
  // Combine: priority first, then secondary (for any other use)
  const allCheshireArticles = [
    ...priorityCheshireArticles,
    ...secondaryCheshireArticles,
  ];

  // Featured article is the first priority Cheshire article (typically Macclesfield)
  const featuredArticle =
    priorityCheshireArticles[0] ||
    secondaryCheshireArticles[0] ||
    articles.find((article) => article.featured) ||
    articles[0];

  // Get 4 articles for Cheshire section (excluding the featured/hero)
  const cheshireSectionArticles = priorityCheshireArticles
  ...
  const handleHeadlineClick = async (headline) => {
    try {
      ...
      let matchingArticle = articles.find(
        (article) => article.title === headline.headline,
      );
  ...
  const fallbackCopyToClipboard = (url) => {
    if (navigator.clipboard) {
      ...
      toast({
        title: "Link Copied!",
        description:
          "Article link copied to clipboard - share it on social media!",
      });
    } else {
      toast({
        title: "Share Link",
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <Helmet>
          <>
            ...
            <meta
              name="description"
              content="Cheshire Today - Your trusted source for local Cheshire news, UK updates, tech, finance, and more."
            />
            ...
            <meta
              property="og:title"
              content="Cheshire Today - Local News & Updates"
            />
            <meta
              property="og:description"
              content="Stay informed with the latest news from Cheshire and across the UK. Your trusted source for local stories."
            />
            <meta
              property="og:image"
              content="https://cheshiretoday.co.uk/social-share.jpg"
            />
        ...
        <Suspense
          fallback={<div className="h-10 bg-red-600 animate-pulse"></div>}
        >
                  ...
                  <div className="lg:col-span-1 space-y-6">
                    ...
                    <Suspense fallback={<LoadingFallback />}>
                      <AffiliateWidgetSidebar
                        category={
                          activeCategory !== "all" ? activeCategory : "default"
                        }
                      />
                    </Suspense>
        ...
        <div className="hidden md:block">
          <Suspense
            fallback={<div className="h-40 bg-gray-100 dark:bg-gray-800"></div>}
          >
            <NewsFooter />
          </Suspense>
        </div>
          ...
          <main className="flex-1 flex items-center justify-center">
            <div className="text-center px-4">
              ...
              <p className="text-gray-600 mb-6">
                Sorry, this article may have been removed or the link is
                incorrect.
              </p>
    ...
    <HelmetProvider>
      <div className="min-h-screen bg-gray-50">
        ...
        <Helmet>
          ...
          <meta
            property="og:url"
            content={`${publicUrl}/article/${articleId}`}
          />
