...
async def get_cold_subscriber_report(
    ...
    recent_days: int = 21,
    sample_limit: int = 50,
    min_accepted_sends: int = 5,
    authorized: bool = Depends(get_admin_auth),
):
    """Aggregate-only, read-only accepted-send evidence; never lifecycle action.

    sample_limit is accepted for legacy callers but no identities are returned.
    """
    from app.newsletter_cold_report import build_cold_report

    try:
        return await build_cold_report(
            db, is_deliverable_newsletter_email,
            days=max(7, min(int(days), 180)),
            recent_days=max(1, min(int(recent_days), 90)),
            min_accepted_sends=max(5, min(int(min_accepted_sends), 100)),
            now=datetime.now(timezone.utc),
        )
    except Exception:
        logger.error("Cold subscriber report unavailable")
        raise HTTPException(status_code=503, detail="Cold subscriber report unavailable") from None
