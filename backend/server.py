...
async def import_hybrid_news(request: HybridNewsRequest = HybridNewsRequest()):
    ...
    try:
        ...
        def is_crime_like(article: dict) -> bool:
            ...
            crime_kw = re.compile(
                r"(cops?|police|officer|mugshot|murder(?:s)?|kill(?:ed|s)?|manslaughter|homicide|stab(?:bing|bed|s)?|shoot(?:ing|s)?|firearm(?:s)?|gunman|rape(?:d)?|sexual assault|sex(?:ual)? offence|indecent|pervert|paedophile|pedophile|\bassault(?:ed|s)?\b|\battack(?:ed|s)?\b|robber(?:y|ies)|burglar(?:y|ies)|arson|charged|arrest(?:ed)?|raid|drug raid|cannabis plants?|prosecut(?:ed|ion)|trial|guilty|sentenc(?:ed|ing)|jailed|jail|prison|convict(?:ed|ion)|inquest(?:s)?)", re.I, )
